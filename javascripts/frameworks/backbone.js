...
(function(){
  ...
  _.extend(Backbone.Collection.prototype, Backbone.Events, {
    ...
    fetch: function(options) {
      options = options ? _.clone(options) : {};
      if (options.parse === undefined) options.parse = true; // changed from true -bec 2/21/11
      var collection = this;
      var success = options.success;
      options.success = function(resp, status, xhr) {
    ...
    prefetch : function(data, options) {
      options || (options = {});
      var collection = this;
      collection.count = data.count;
      collection.has_more = data.count > data.collection.length;
      collection['reset'](collection.parse(data), options);
    }, 
  ...
  _.extend(Backbone.History.prototype, Backbone.Events, {
    ...
    navigate: function(fragment, options) {
      ...
      if (window.unsaved_changes && !confirm(window.unsaved_prompt)) {
        return false;
      } else {
      ...
      // clear the global change state
      if (window.unsaved_changes) { window.unsaved_changes = false; }
